"""Thin async client for the Resilio Sync WebUI API, with no HA dependency."""
...
class ResilioClient:
    ...
    Takes a caller-provided ``aiohttp.ClientSession`` rather than opening its
    own, so a single session can be shared across calls or, in Home
    Assistant, sourced from ``async_get_clientsession(hass)``.
    """
